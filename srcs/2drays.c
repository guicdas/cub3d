...
/*   Updated: 2024/06/03 16:39:00 by gcatarin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_player_direction(int x1, int y1, int color)
{
	...
	while (i > 0)
	{
		mlx_pixel_put(d()->mlx, d()->win_ptr, (x1 + d()->player_dx*i), (y1 + d()->player_dy*i), color);
		i -= 0.2;
	}
	// printf("x2: %d, y2: %d\n",  x1+ d()->player_dx*5, y1 + d()->player_dy*5);
}
...
void draw_horizontal_rays()
{
	...
	for(int r = 0 ; r < 1 ; r++)
	{
		// printf("ang-%f, x-%f, y-%f\n", d()->player_a * 180 / PI, d()->player_x, d()->player_y);
		dof = 0;
		yo = 8;
		if (d()->player_a > PI)
		{
